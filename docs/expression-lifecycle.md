...
at *build* time, and so is the only materialized copy of its result. A
worthy entry's **snapshot** (a parquet file of its rows, which tallyman writes
itself) is written during the build, not on first read. Only two things are
populated *lazily*, at first view: Buckaroo's stat cache on first `/load_expr`,
and the view build directory a worthy entry's grid is handed. So "build" and
"first view" each write different things, and a restart re-runs the view-time
work against caches that survived on disk.
...
`_run_and_record` → `build_and_persist(project, code, prompt)`.

## 2. Build + execute (`build_and_persist`)

The whole build holds the project's write lock (`catalog_state.project_lock`), so
there is one writer at a time per project. In order:
...
1. **Import the user code** in a fresh module scope (`_import_script`). During
   ...
   that copy is already on disk. `tracked_expr_from_alias` records each parent
   edge, and makes the parent's files exist first (`ensure_materialized`),
...
2. **Check, classify, rewrite, compile.** The build refuses a raw
   ...
   `__row_order`, and a cheap entry that drops that column. It gives a worthy
...
3. **Idempotency check**: if `<entry>` already exists for this hash, nothing is
   rebuilt — the prompt is appended as a re-run event and the existing
   `BuildResult` is returned. (This is the cheap path that Part 2 leans on.)
...
5. **Execute.** This is the one place the entry's query runs, and what it writes
   ...
   loads the entry's frozen build, rebinds it onto a single-partition
   ...
   row-level evaluation (a bad cast or UDF fails here, in tallyman's process)
   and yields the exact row count, but keeps no rows. No `result.parquet` is
   written either way.
...
| Source digests | `<project>/artifacts/source_digests.json` (stat-keyed memo) + the manifest's `sources` map (`cas` and `salt`) | `source_identity`, step 1 |
...
expect_exists=False)`, which writes a line to the tracked
...
`/internal/notify`, which publishes an SSE event to any connected SPA
(`app.py`). The viewer's catalog list refreshes; nothing touches Buckaroo yet.
The git commit for all of this lands when the tool returns, at the checkpoint
(§2 step 7).
...
The grid loads separately. `LazyBuckarooEmbed` waits until the grid scrolls near
the viewport, then polls `GET /{project}/api/session/{hash}` (`get_session`)
until it returns a websocket URL.
...
3. **Pick what Buckaroo is handed.** For a **worthy** entry, `ensure_view_build`
   ...
   a random tmp dir, gated by a `.complete` marker. Both paths are stable
   because Buckaroo's stat-cache keys include the build directory's path; a
   random path would miss the stat cache on every restart.
...
5. **POST `/load_expr`** with `session=entry-<project>-<content_hash>`,
   `build_dir` (from step 3), `project_root=<artifacts_dir>` (where Buckaroo
   scans for project-authored stat/post-processing klasses),
   `cache_storage_path=<stat_cache>`, and `row_order_column="__row_order"`. A
   promoted diff entry adds `column_config_overrides`, and the companion adds
   `telemetry_url` when it knows its own address. Buckaroo loads the xorq
   expression, creates the session, and returns its `session` id.
...
`caching.md`). Buckaroo pages in its own process, and pages an entry by the
`row_order_column` it was told; builds of Buckaroo that predate that hint ignore
it (buckaroo-data/buckaroo#974).
...
request returns the same rows), both sides of a diff, chaining a child recipe, and
post-processing.
...
finds `<entry>` already present, appends the prompt, and returns the existing
`BuildResult`. No execution, no materialization, and no new file: the ordered
...
recompute, no stat-cache touch. The SPA reuses the same websocket.

## C. Revisiting after a restart (disk-warm, RAM-cold)

When Buckaroo restarts, the companion detects a new `started` timestamp from
`/health` and clears its in-RAM bookkeeping of diff sessions
(`_reset_session_bookkeeping_if_restarted`) — but it touches nothing on disk.
There is no entry-session record to clear. So on the next visit:
...
path deletes it, and (b) the build path is stable, so the stat-cache key is
identical across processes.

The only things that *invalidate* the stat cache are a klass-changing event — a
summary-stat / post-processing / display-config change, a project reset, or a
recalc — and an unfaithful heal. The first group routes through
`reload_project_sessions`, which POSTs `/reload_expr/<id>` for every entry of the
project (a 404 means the grid is not open) and clears the stat cache of each grid
it reloaded (`_clear_stat_cache`). A plain restart is not such an event.
...
- **Build time** (`build_and_persist`): during the recipe import, the ordered
  ...
  `data/.cas/<digest>` clone; then the snapshot under
  `compute_cache/result_cache/` (worthy entries only), `schema.json` /
  `manifest.json`, source digests (`cas` and `salt`), `expr.py`, and the prompt
  history.
- **Checkpoint** (when the tool returns, `checkpoint_catalog`): the recipe zip
  `entries/<hash>.zip`, the `aliases.jsonl` / `entries.jsonl` tracked surface,
  and one git commit + step tag.
- **First view** (`load_session` → `/load_expr`): `.buckaroo_stat_cache/`, and the
  stable expanded build (cheap entry) or view build (worthy entry). No session
...
the stat cache). Only the user
deletes a file (the Cache page; a pinned snapshot is refused). A `reset_to`
