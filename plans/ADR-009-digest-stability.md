...
- **Status:** Proposed (2026-09-18). Amends
  ...
  snapshot bytes") and `plans/ADR-006-read-path-loads-builds.md` D5
  ("`result_digest` keeps its file-hash definition"). D5's canonical sort is
...
- **Affected code:** `src/tallyman_xorq/result_cache.py`
  ...
  writer that `plans/ADR-007-tallyman-owned-materialization.md` D4 introduces.
...
cache, evicts its session, and pins and badges the entry (ADR-006 D7, D10,
D12). That response is right for a recipe that calls `sample()`. It is
...
was written before a mismatch became loud. With D7, D10 and D12 in place, an
...
`materialize` (ADR-007 D4) executes on a connection configured with
...
setting, because a long bake must not share a context with page reads.
...
43-column read at 2.5 s against 6.9 s, on the 11.8M-row parking file. A bake
runs once per entry and once per heal, never on a read.
...
anyway. The sort is what puts the author's keys first in stored order
(ADR-008 D1 relies on it), and it keeps the result independent of the hash
...
and verify. This is the "one derivation route" rule behind ADR-006 D8. The
...
The digest stays order-sensitive, so D5's canonical sort is still what makes it
reproducible.

This reverses part of ADR-006 D5's reasoning. D5 rejected the multiset digest
...
Tallyman's writer (ADR-007 D4) regroups the record-batch stream into row groups
of 1,048,576 rows, combines each row group into contiguous arrays, and writes
zstd level 3, parquet format 2.6, statistics on. For the spike's rows that is
...
depends on that after D2, and it means two bakes can still be compared with
`cmp` when debugging. Because of D2 these settings can change later without
touching any digest.
...
Every worthy entry's digest is recomputed by the corpus rebuild of ADR-007 D9.
...
- A float-aggregate entry heals to the same digest, and a pyarrow upgrade no
  longer flags anything. The D10 and D12 responses are left for the causes they
  were designed for.
- Bakes and heals are slower, by about 3x on aggregation at spike scale and up
...
1. **Rebuild time under single-partition bakes.** Decides D1 against D1b.
...
3. **Types parquet cannot store as given.** A `timestamp[s]` column comes back
   ...
   schema is recorded from the snapshot.
