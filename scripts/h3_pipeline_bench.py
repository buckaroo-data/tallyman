...
def phase6():
    ...
    manhattan_zones = zone_lookup.filter(pl.col("Borough") == "Manhattan")["LocationID"].to_list()
    df = (
        taxi_hexed.filter(pl.col("pu_hex").is_in(bike_hexes))
...
def _make_xorq_query():
    ...
    q = joined.group_by("pu_hex").agg(joined.pu_hex.count().name("n")).order_by(_.n.desc())
    return q.execute()
