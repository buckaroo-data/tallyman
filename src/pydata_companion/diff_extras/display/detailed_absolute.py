...
def _layout(result, sd, keep_suffix, drop_suffix):
    ...
    result = [
        c for c in result
        if not (c.get("header_name", "").endswith(keep_suffix) and _all_zero(sd, c))
    ]
    delta_alias = {
...
class DiffDetailedAbsoluteStyling(DefaultMainStyling):  # noqa: F821
    """'detailed_absolute' display: each value column next to its ``_abs_delta``
    (all-zero deltas dropped), ``_pct_delta`` hidden, colored by abs_delta."""
    ...
    def style_columns(cls, sd, df):
        ...
        return _layout(result, sd, "_abs_delta", "_pct_delta")
