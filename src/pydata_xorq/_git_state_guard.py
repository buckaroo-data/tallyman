...
from the long-lived, multithreaded pydata server it can die with ``SIGSEGV`` (a
...
_safe_get_git_state._pydata_guarded = True  # type: ignore[attr-defined]


def install_git_state_guard() -> None:
    """Idempotently replace ``logging_utils.get_git_state`` with the safe version.

    Lazy xorq import so importing pydata_xorq doesn't pull in xorq eagerly.
    """
    from xorq.common.utils import logging_utils as lu

    if getattr(lu.get_git_state, "_pydata_guarded", False):
        return
