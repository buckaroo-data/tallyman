...
def _parent_v1_code(project: str, src: str) -> str:  # full orders
    return f"""
...
def _parent_v2_code(project: str, src: str) -> str:  # same source, boots only — fewer rows
    return f"""
from tallyman_xorq.io import tracked_expr_from_alias
t = tracked_expr_from_alias({src!r}, project={project!r})
...
def _lineage(project: str, src: str, monkeypatch, child_code: str) -> tuple[str, str]:
    ...
    a1 = catalog_create("trips", _parent_v1_code(project, src))["hash"]
    b1 = catalog_create("by_region", child_code)["hash"]
    catalog_revise("trips", _parent_v2_code(project, src))
...
def test_cheap_historical_read_uses_recorded_parent(project, orders_src, monkeypatch):
    ...
    """
    b1, _b2 = _lineage(project, orders_src, monkeypatch, _cheap_child_code())
...
def test_worthy_revisions_resolve_distinct_snapshots(project, orders_src, monkeypatch):
    ...
    """
    b1, b2 = _lineage(project, orders_src, monkeypatch, _worthy_child_code())
...
def test_untouched_historical_entry_verifies_faithful(project, orders_src, monkeypatch):
    ...
    """
    b1, _b2 = _lineage(project, orders_src, monkeypatch, _worthy_child_code())
    assert verify_result_faithful(project, b1) is True


def test_diff_of_two_revisions_is_not_a_self_join(project, orders_src, monkeypatch):
...
def test_cold_cache_read_reproduces_build_bytes(project, orders_src, monkeypatch):
...
def _worthy_parent_code(project: str, src: str) -> str:  # Aggregate at the parent → worthy parent
    return f"""
from tallyman_xorq.io import tracked_expr_from_alias
t = tracked_expr_from_alias({src!r}, project={project!r})
expr = t.group_by("region").aggregate(n=t.count(), total=t.price.sum())
...
def test_missing_build_is_hard_error(project, orders_src):
    ...
    """
    h = catalog_create("plain", _parent_v1_code(project, orders_src))["hash"]
...
def test_chained_child_reads_after_every_file_is_evicted(project, orders_src):
    ...
    evicted, ``cached_result_expr`` (the one canonical read) makes the parent's snapshot exist and verify again, and
    under it the imported source version's snapshot, which is re-created from the clone of the bytes (ADR-011 D1),
    before the child's plan executes.
    """
    parent = catalog_create("trips", _worthy_parent_code(project, orders_src))["hash"]
    child_code = """
from tallyman_xorq.io import tracked_expr_from_alias
t = tracked_expr_from_alias("trips")
...
def test_unfaithful_heal_wipes_stat_cache(project, orders_src):
    ...
    """
    h = catalog_create("by_region", _worthy_parent_code(project, orders_src))["hash"]
