...
def _random_recipe(project: str) -> str:
...
def test_a_reset_forward_restores_the_manifest_that_matches_the_snapshot_of_an_entry_added_again(
    ...
    first = build_and_persist(project, _random_recipe(project))
    ...
    assert build_and_persist(project, _random_recipe(project)).content_hash == h
    s2 = cs.checkpoint_catalog(project, "s2")
